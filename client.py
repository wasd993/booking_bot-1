...
from database import (
    get_setting, get_active_specialist,
...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    active_specialist = await get_active_specialist()
    if active_specialist:
        specialist = active_specialist["name"]
        profession = active_specialist["profession"]
    else:
...
async def cb_choose_time(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    active_specialist = await get_active_specialist()
    specialist_id = active_specialist["id"] if active_specialist else None
    specialist_name = active_specialist["name"] if active_specialist else ""
...
async def my_appointments(message: Message) -> None:
    ...
    for apt in apts:
        ...
        status_text = "активна" if status == "active" else status
        lines.append(
            f"• <b>{d}.{m}.{y}</b> в <b>{apt['time']}</b>  ⭐️ {stars} — <i>{status_text}</i>"
        )
...
async def cb_confirm_cancel(callback: CallbackQuery) -> None:
    ...
    appointment_dt = datetime.fromisoformat(f"{apt['date']} {apt['time']}")
    refund_allowed = appointment_dt - datetime.now() > timedelta(hours=12)
    specialist_name = apt.get("specialist_name", "")
    ...
    if stars > 0:
        if refund_allowed:
            ...
            specialist_note = f" к специалисту <b>{specialist_name}</b>" if specialist_name else " к специалисту"
            refund_note = (
                f"\n⚠️ Возврат <b>{stars} ⭐️</b> невозможен — обратитесь{specialist_note}."
            )
...
async def cb_do_cancel(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    refunded = False
    refund_failed = False
    if refund_allowed and apt.get("charge_id") and apt.get("stars_paid", 0) > 0:
        try:
    ...
    if apt.get("stars_paid", 0) > 0:
        if refunded:
            ...
            specialist_note = f" к специалисту <b>{specialist_name}</b>" if specialist_name else " к специалисту"
            refund_text = f"\n⚠️ Возврат невозможен — обратитесь{specialist_note}."
